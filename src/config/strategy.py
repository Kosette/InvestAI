...
class PullbackConfig(BaseModel):
    ...
    threshold: float = Field(..., description="回调幅度阈值，如 0.03 表示 3%")


class BreakoutConfig(BaseModel):
    resistance_window: int = Field(..., description="阻力位计算回看窗口（交易日数）")
    buffer: float = Field(..., description="突破确认缓冲比例，如 0.005 表示 0.5%")
...
class VolumeConfig(BaseModel):
    ...
    min_ratio: float = Field(..., description="当前成交量 ≥ 均量 × 该比例")
